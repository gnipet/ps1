...
char* playfair_encrypt(const char* key, const char* text) {
    ...
    for (int i = 0; i < len; i++) {       // Prechádzame TEXT
        if (prepared_text[i] != ' ') {      // Ak je tam niej emedzera 
            prepared_text[j++] = toupper(prepared_text[i]); // Zmeň na veľké
    ...
    for (int i = 0; i < len; i += 2) { // Robíme po dvojiciach 
        ...
        int row_a, col_a, row_b, col_b;   // Deklarácia premenných 
    ...
    return encrypted;       // návrat zašifrovaného textu
}
